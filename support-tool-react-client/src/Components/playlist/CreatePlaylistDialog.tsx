...
export const CreatePlaylistDialog: React.FC<CreatePlaylistDialogProps> = ({ open, onClose, onSuccess }) => {
  const { interceptAction } = useActionIntercept();
  const [type, setType] = useState('MDO_FEATURED_COURSES');
  ...
  const handleCreate = async () => {
    ...
    interceptAction('PLAYLIST', actionPayload, async (auditData: any) => {
      setSaving(true);
      try {
        await playlistService.createPlaylist(auditData);
        onSuccess();
        onClose();
      } catch (err: any) {
        const msg = err?.response?.data?.error?.message || err?.response?.data?.responseMessage || err.message || 'Failed to create playlist';
        setError(msg);
      } finally {
        setSaving(false);
      }
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent dividers>
        ...
        <TextField
          ...
          sx={{ mb: 2 }}
          helperText='e.g. MDO_FEATURED_COURSES'
        />
        ...
        {children.length > 0 && (
          <Typography variant="caption" color="text.secondary">{children.length} content(s) added</Typography>
        )}
      </DialogContent>
      <DialogActions>
