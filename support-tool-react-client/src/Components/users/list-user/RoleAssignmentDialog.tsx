...
interface RoleAssignmentDialogProps {
  ...
  onRoleAssign: (userId: string, orgId: string, roles: string[]) => Promise<void>;
}
...
export const RoleAssignmentDialog: React.FC<RoleAssignmentDialogProps> = ({
    ...
    if (open && user) {
      ...
      if (user.roles && Array.isArray(user.roles) && user.roles.length > 0) {
        ...
        if (orgWithRoles && orgWithRoles.roles && Array.isArray(orgWithRoles.roles)) {
          setSelectedRoles(orgWithRoles.roles);
        } else {
          // If no matching organization or no roles in matching organization
          setSelectedRoles(['PUBLIC']); // Default to PUBLIC role as fallback
        }
      } else {
        // If no roles information is available at all
        setSelectedRoles(['PUBLIC']); // Default to PUBLIC role
      }
    }
  }, [open, user]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      await onRoleAssign(user.identifier, user.rootOrgId, selectedRoles);
