...
import { 
    Box, CircularProgress, Typography, Button, Paper, 
    Tooltip, Snackbar, Alert, TextField, FormControl,
...
export const Edit = () => {
    ...
    const toggleValueType = () => {
        ...
        if (isJsonValue) {
            // From JSON to text - stringify if needed
            if (typeof input !== 'string') {
                setInput(JSON.stringify(input));
            }
        } else {
            // From text to JSON - try to parse
            try {
                const parsed = JSON.parse(input as string);
                setInput(parsed);
            } catch (e) {
                // If parsing fails, keep as is and notify
                setNotification({
                    open: true,
                    message: 'Current value is not valid JSON',
                    severity: 'info'
                });
            }
        }
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                    {/* <FormControl size="small" sx={{ minWidth: 120 }}>
                        ...
                        <Select
                            ...
                            value={isJsonValue ? "json" : "text"}
                            label="Value Type"
                            onChange={(e) => {
                                if ((e.target.value === "json" && !isJsonValue) || 
                                    (e.target.value === "text" && isJsonValue)) {
                                    toggleValueType();
                                }
                            }}
                            size="small"
            ...
            <Box position="relative" sx={{ height: isJsonValue ? 'calc(100vh - 180px)' : 'auto', minHeight: isJsonValue ? '400px' : 'auto' }}>
                {isLoading && (
                    <Box 
                        position="absolute" 
                        top={0} 
